...
function Particles() {
  ...
  return (
    <instancedMesh ref={meshRef} args={[undefined, undefined, count]}>
      ...
      <meshBasicMaterial color="#00e5ff" transparent opacity={0.4} blending={THREE.AdditiveBlending} depthWrite={false} />
    </instancedMesh>
  );
}
...
function WaveGrid() {
  ...
  useFrame((_, delta) => {
    if (gridRef.current) {
      gridRef.current.rotation.x = Math.PI / 2.2; // Tilt it down
  ...
  return (
    <group ref={gridRef}>
      <gridHelper args={[100, 100, "#0088ff", "#0088ff"]} material-transparent material-opacity={0.15} />
    </group>
  );
}
